...
require_once PAY_ROOT.'inc/lib/AopCertClient.php';
...
class AlipayService {
	...
	function __construct($alipay_config){
		...
		$this->app_auth_token = $alipay_config['app_auth_token'];

		$this->alipayCertPath = $alipay_config['alipayCertPath'];
		$this->appCertPath = $alipay_config['appCertPath'];
		$this->rootCertPath = $alipay_config['rootCertPath'];
	...
	protected function aopclientRequestExecute($request, $token = NULL, $appAuthToken = NULL) {
		...
		// 开启页面信息输出
		$aop->debugInfo=true;
		$result = $aop->execute($request,$token,$this->app_auth_token);
	...
	protected function aopclientRequestPageExecute($request, $token = NULL, $appAuthToken = NULL) {
		...
		// 开启页面信息输出
		$aop->debugInfo=true;
		$result = $aop->pageExecute($request,$token,$this->app_auth_token);
